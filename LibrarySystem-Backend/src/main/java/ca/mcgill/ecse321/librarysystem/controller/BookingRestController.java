	 ...
	 */
	 ...
	 public ResponseEntity updateUserofBookingByID(@RequestParam String bid, @RequestParam String libId) {
		...
		private HourDto convertToDto(Hour h){
			...
			return new HourDto(h.getId(), h.getEvent(), h.getWeekday(),h.getStartTime(),h.getEndTime(),convertToDto(h.getEmployee()),convertToDto(h.getCalendar()), HourDto.Type.valueOf(h.getType().toString()));
